...
### Q1: 카나리 배포와 A/B 테스트의 차이는?
**A**: 
- **카나리 배포**: 새 버전의 안정성 검증이 목적. 트래픽을 점진적으로 증가시키며 에러율/응답시간 모니터링
- **A/B 테스트**: 기능의 효과 측정이 목적. 사용자를 무작위로 나눠 전환율/체류시간 등 비즈니스 지표 비교

### Q2: 블루-그린 배포 시 데이터베이스는 어떻게 처리하나요?
**A**: 
1. **읽기 전용 모드**: 전환 직전 DB를 읽기 전용으로 변경
2. **스키마 호환성**: 새 버전이 구 스키마와 호환되도록 설계
3. **마이그레이션 분리**: DB 마이그레이션을 배포와 분리하여 사전 실행
4. **롤백 계획**: 데이터 변경 시 롤백 스크립트 준비

### Q3: 카나리 배포 실패 시 자동 롤백되나요?
**A**: Argo Rollouts의 Analysis 기능을 사용하면 자동 롤백 가능합니다. Prometheus 메트릭(에러율, 응답시간)을 기준으로 실패 판단 시 자동으로 이전 버전으로 롤백합니다.

### Q4: 소규모 서비스에도 복잡한 배포 전략이 필요한가요?
**A**: 
- **트래픽 < 1000 req/day**: 롤링 배포로 충분
- **트래픽 1000~10000**: 카나리 배포 권장
- **트래픽 > 10000 또는 금융/결제**: 블루-그린 또는 카나리 필수

### Q5: 롤링 배포의 maxUnavailable과 maxSurge는 무엇인가요?
**A**: 
- **maxUnavailable**: 업데이트 중 사용 불가능한 최대 Pod 수 (예: 25% = 4개 중 1개)
- **maxSurge**: 업데이트 중 추가 생성 가능한 최대 Pod 수 (예: 25% = 4개 중 1개 추가)
- 둘 다 0일 수 없음 (배포 불가능)

### Q6: 카나리 배포 시 트래픽 비율은 어떻게 결정하나요?
**A**: 
1. **1-5%**: 초기 검증 (에러 감지)
2. **10-25%**: 성능 검증 (응답시간, 리소스)
3. **50%**: 부하 검증 (실제 트래픽 부하)
4. **100%**: 전체 전환
- 각 단계마다 5-30분 대기하며 메트릭 모니터링

### Q7: 블루-그린 배포는 리소스가 2배 필요한가요?
**A**: 네, 전환 시점에 구 버전(블루)과 신 버전(그린)이 동시에 실행되어 일시적으로 2배 리소스가 필요합니다. 전환 완료 후 구 버전을 종료하면 정상 수준으로 돌아옵니다.

### Q8: 배포 전략 선택 시 가장 중요한 기준은?
**A**: 
1. **비즈니스 영향도**: 장애 시 손실 규모
2. **롤백 속도**: 얼마나 빨리 복구해야 하는가
3. **리소스 비용**: 블루-그린은 2배 리소스 필요
4. **팀 역량**: 복잡한 전략은 운영 부담 증가

### Q9: 카나리 배포 중 문제 발견 시 어떻게 하나요?
**A**: 
1. **자동 롤백**: Analysis 설정 시 자동으로 이전 버전 복구
2. **수동 중단**: `kubectl argo rollouts abort` 명령으로 즉시 중단
3. **트래픽 고정**: 현재 비율에서 멈추고 원인 분석
4. **점진적 감소**: 트래픽을 다시 줄여가며 안정화

### Q10: 여러 배포 전략을 동시에 사용할 수 있나요?
**A**: 네, 가능합니다. 예:
- **마이크로서비스 A**: 카나리 배포 (사용자 영향 큼)
- **마이크로서비스 B**: 롤링 배포 (내부 서비스)
- **마이크로서비스 C**: 블루-그린 (금융 거래)
각 서비스의 특성에 맞는 전략 선택
