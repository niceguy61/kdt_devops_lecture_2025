...
```mermaid
graph LR
    subgraph "EC2 비용 패턴 (계단식)"
        A1[트래픽 증가] --> B1[인스턴스 추가]
        B1 --> C1[비용 급증]
        C1 --> D1[트래픽 감소]
        D1 --> E1[인스턴스 유지]
        E1 --> F1[비용 누수]
...
| 트래픽 수준 | EC2 비용 (t3.small 2대) | Lambda 비용 | 비고 |
|------------|------------------------|-------------|------|
| **낮음** (10K req/day) | $30/월 (고정) | $2/월 | Lambda 93% 저렴 |
| **중간** (100K req/day) | $60/월 (4대) | $20/월 | Lambda 67% 저렴 |
| **높음** (1M req/day) | $120/월 (8대) | $200/월 | EC2 40% 저렴 ⚠️ |
| **매우 높음** (10M req/day) | $240/월 (16대) | $2,000/월 | EC2 88% 저렴 ⚠️ |

**💡 핵심 인사이트**:
- **Lambda 장점**: 낮은~중간 트래픽에서 비용 누수 최소화
- **EC2 장점**: 높은 트래픽에서 예측 가능한 비용
