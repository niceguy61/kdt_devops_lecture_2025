...
---
xychart-beta
    title "EC2 vs Lambda 비용 비교 (트래픽 증가에 따른)"
    x-axis "일일 요청 수" [0, 10K, 50K, 100K, 500K, 1M, 5M, 10M]
    y-axis "월간 비용 ($)" 0 --> 2500
    line "EC2 (계단식)" [30, 30, 60, 60, 120, 120, 240, 240]
    line "Lambda (선형)" [0, 2, 10, 20, 100, 200, 1000, 2000]
```

**비용 비교 시나리오**:

| 트래픽 수준 | EC2 비용 (t3.small) | Lambda 비용 | 비고 |
|------------|---------------------|-------------|------|
| **낮음** (10K req/day) | $30/월 (2대 고정) | $2/월 | Lambda 93% 저렴 ✅ |
| **중간** (100K req/day) | $60/월 (4대 고정) | $20/월 | Lambda 67% 저렴 ✅ |
| **높음** (1M req/day) | $120/월 (8대 고정) | $200/월 | EC2 40% 저렴 ⚠️ |
| **매우 높음** (10M req/day) | $240/월 (16대 고정) | $2,000/월 | EC2 88% 저렴 ⚠️ |

**💡 핵심 인사이트**:
- **Lambda 장점**: 
  - 낮은~중간 트래픽에서 비용 누수 최소화
  - 트래픽 감소 시 비용 즉시 감소 (선형)
  - 유휴 시간 비용 없음
- **EC2 장점**: 
  - 높은 트래픽에서 예측 가능한 비용
  - 지속적 사용 시 더 경제적 (계단식이지만 총액 낮음)
