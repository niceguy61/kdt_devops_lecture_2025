...
| 유연한 확장 | 수직(크기), 수평(수량) 확장 | - |
| 마이그레이션 | 기존 애플리케이션 쉽게 이전 | - |
| 과금 모델 | On-Demand, Reserved, Spot (최대 90% 할인) | - |
| **⚠️ 단점** | | |
| 서버 관리 | OS 패치, 보안 업데이트, 모니터링 필요 | Lambda, Fargate (서버리스) |
| 비용 예측 | 사용량 변동 시 비용 변동 | Lightsail (고정 가격) |
| 학습 곡선 | 인스턴스 타입, 네트워킹, 보안 설정 | Elastic Beanstalk (PaaS) |
| 중지 시 비용 | 인스턴스 중지해도 EBS 비용 발생 | 불필요 시 종료 |
...
| 모델 | 과금 방식 | 할인율 | 약정 | 적합 용도 |
|------|----------|--------|------|----------|
| **On-Demand** | 초 단위 (최소 60초) | 0% | 없음 | 개발/테스트, 예측 불가 |
| **Reserved** | 1-3년 약정 | 최대 72% | 1-3년 | 안정적 프로덕션 |
| **Spot** | 경매 방식 | 최대 90% | 없음 (회수 가능) | 배치, 분석, 중단 가능 |
| **Savings Plans** | 시간당 사용량 약정 | 최대 72% | 1-3년 | 유연한 장기 워크로드 |
- 최대 72% 할인
- 인스턴스 타입 변경 가능
- 용도: 유연한 장기 워크로드
