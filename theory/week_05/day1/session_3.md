...
> **정의**: EC2 인스턴스에 연결되는 블록 수준 스토리지 볼륨
...
graph TB
    subgraph "EBS 아키텍처"
        A[EC2 Instance]
        B[Root Volume<br/>EBS gp3]
        C[Data Volume 1<br/>EBS io2]
        D[Data Volume 2<br/>EBS st1]
        E[EBS Snapshot<br/>S3 저장]
    end
...
| 볼륨 타입 | 용도 | IOPS | 처리량 | 가격 | 프리티어 |
|----------|------|------|--------|------|---------|
| **gp3** | 범용 SSD | 3,000-16,000 | 125-1,000 MB/s | 중간 | ✅ 30GB |
| **gp2** | 범용 SSD (이전) | 100-16,000 | 최대 250 MB/s | 중간 | ✅ 30GB |
| **io2** | 고성능 SSD | 100-64,000 | 최대 1,000 MB/s | 높음 | ❌ |
| **st1** | 처리량 최적화 HDD | - | 최대 500 MB/s | 낮음 | ❌ |
| **sc1** | Cold HDD | - | 최대 250 MB/s | 매우 낮음 | ❌ |

**💡 Week 5 권장**: **gp3** (범용 SSD)
- 가격 대비 성능 우수
- 프리티어 30GB 제공
- IOPS와 처리량 독립 조정 가능
- 대부분의 워크로드에 적합
...
1. **스냅샷 (Snapshot)**:
   ...
   sequenceDiagram
       ...
       participant S3 as S3 (백업)
       participant New as New Volume
       
       EBS->>S3: 1. 스냅샷 생성
       Note over S3: 증분 백업<br/>첫 번째: 전체<br/>이후: 변경분만
       S3->>New: 2. 스냅샷에서 복원
       Note over New: 다른 AZ/Region<br/>복원 가능
   ```
...
**⚠️ EBS 주의사항**:
- **AZ 종속**: 같은 AZ 내 EC2에만 연결 가능
- **단일 연결**: 하나의 EC2에만 연결 (io2는 다중 연결 가능)
- **삭제 정책**: 인스턴스 종료 시 Root Volume 자동 삭제 (변경 가능)
- **비용**: 프로비저닝된 용량에 대해 과금 (사용량 무관)
