...
**3단계: EC2 배포 (15분)**
- Public EC2 2대 배포 (Nginx 설치)
- Private EC2 2대 배포

### Phase 2: 문제 해결 (10분)

**⚠️ 의도적 오류 포함**:
Challenge 환경에는 다음과 같은 오류가 포함되어 있을 수 있습니다:
- Route Table 연결 누락
- Security Group 규칙 오류
- Nginx 서비스 미실행

**💡 힌트**:
- VPC Resource Map으로 네트워크 구조 확인
...
**검증 체크리스트**:
- [ ] 브라우저에서 Public EC2 접속 (http://[Public-IP])
- [ ] Public EC2에서 Private EC2 SSH 접속
- [ ] Private EC2 간 Ping 테스트
- [ ] Private EC2에서 외부 접속 차단 확인
...
### 시간 관리
- **네트워크 구성**: 15분 (서두르지 말고 정확히)
- **보안 그룹**: 5분 (Lab 2 참고)
- **EC2 배포**: 15분 (User Data 활용)
- **문제 해결**: 10분 (체계적 진단)
- **검증**: 5분 (체크리스트 확인)
